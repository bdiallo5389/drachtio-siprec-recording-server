  ...
  return payloadParser(opts)
    ...
    .then((opts) => {
      ...
      const headers = {
        ...
        'X-associate-time'       : siprec.associate_time                                        || 'undefined',
      ...
      return tryConnect()
        .then((dialogs) => {
            ...
            return setDialogHandlers(opts.logger, dialogs, clientName, successfulServer);
         })
    })
    .catch((err) => {
...
function setDialogHandlers(logger, dialogs, clientName, serverName) {
  ...
  uas
    ...
    .on('update', async (req, res) => {
      ...
      try {
        const { apiCallIP, port, route, apiCallTimeout } = config.get('aa');
        // Parse payload
