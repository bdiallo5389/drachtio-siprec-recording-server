  ...
  return payloadParser(opts)
    ...
    .then((opts) => {
      ...
      function tryConnect() {
        ...
        // Guard against infinite loop when all configured URIs are identical:
        // tried.size never grows inside the loop, so cap iterations at maxAttempts.
        let searchCount = 0;
        do {
          fsUri = getAvailableFreeswitch();
        } while (tried.has(fsUri) && ++searchCount < maxAttempts);

        if (tried.has(fsUri)) {
          opts.logger.warn({ tried: [...tried] }, 'All media servers exhausted (duplicate URIs) — declining call');
