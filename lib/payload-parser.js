...
const { extractCsInfo, extractParticipantAssoc } = require('./siprec-metadata-utils');
      ...
      try {
        if (typeof result === 'object' && Object.keys(result).length === 1) {
          ...
          // If only one participant has x_part_leg, the other role is deduced from remaining participants
          {
            ...
            // sdp1 was assigned to the media block whose stream label falls in the
            // "callerLabel" set (['1','10','a_leg','inbound']) — i.e. to the participant
            // identified as caller by the stream-label heuristic (streamCallerPid).
            // When x_part_leg determines a different true caller (finalCallerPid ≠ streamCallerPid),
            // the SDP blocks must be swapped so that sdp1 always carries the caller's stream
            // (used for leg 1 toward the media server) and sdp2 carries the callee's stream
            // (stored in Redis for the leg 2 response).
            if (finalCallerPid && streamCallerPid && finalCallerPid !== streamCallerPid) {
              const tmp = opts.sdp1;
              opts.sdp1 = opts.sdp2;
              opts.sdp2 = tmp;
              debug('[parser] [SRC] SDP blocks swapped: x_part_leg caller (%s) differs from stream-label caller (%s) — sdp1 now carries caller SDP',
                finalCallerPid, streamCallerPid);
            }
          }
          ...
          opts.siprec = {
            ...
            cs_direc,
            associate_time:  obj[`${prefix}sessionrecordingassoc`]?.[0]?.[`${prefix}associate-time`]?.[0]  || null,
            dissociate_time: obj[`${prefix}sessionrecordingassoc`]?.[0]?.[`${prefix}dissociate-time`]?.[0] || null,
            call_id_1,
            call_id_2,
          };
          debug('[parser] [SRC] siprec: group_id=%s session_id=%s cs_direc=%s call_id_1=%s call_id_2=%s',
            opts.siprec.group_id, opts.siprec.session_id, opts.siprec.cs_direc, call_id_1, call_id_2);

          // x_call_id_1 maps to the caller-side CS leg (client), x_call_id_2 to the callee-side
          // CS leg (agent). Per-participant associate/dissociate time and x_event (from
          // participantsessionassoc) are attached the same way.
          const assocMap = extractParticipantAssoc(obj, prefix);
          if (opts.caller.participantId) {
            const a = assocMap.get(opts.caller.participantId);
            opts.caller.callId         = call_id_1;
            opts.caller.associateTime  = a?.associate_time;
            opts.caller.dissociateTime = a?.dissociate_time;
            opts.caller.xEvent         = a?.x_event;
          }
          if (opts.callee.participantId) {
            const a = assocMap.get(opts.callee.participantId);
            opts.callee.callId         = call_id_2;
            opts.callee.associateTime  = a?.associate_time;
            opts.callee.dissociateTime = a?.dissociate_time;
            opts.callee.xEvent         = a?.x_event;
          }
          debug('[parser] [SRC] caller enrichment=%j callee enrichment=%j',
            { callId: opts.caller.callId, associateTime: opts.caller.associateTime, dissociateTime: opts.caller.dissociateTime, xEvent: opts.caller.xEvent },
            { callId: opts.callee.callId, associateTime: opts.callee.associateTime, dissociateTime: opts.callee.dissociateTime, xEvent: opts.callee.xEvent });
        }
      }
      catch (err) {
        reject(err);
