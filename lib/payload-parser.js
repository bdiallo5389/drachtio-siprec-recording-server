      ...
      try {
        if (typeof result === 'object' && Object.keys(result).length === 1) {
          ...
          obj[`${prefix}stream`].forEach((s) => {
            ...
            participants[sender].label = s[`${prefix}label`][0];

            // '10' is the BroadWorks-specific label for the a-leg (SIP OPTIONS BroadWorks 24.0+)
            if (-1 !== ['1', '10', 'a_leg', 'inbound'].indexOf(participants[sender].label)) {
              opts.caller.aor = participants[sender].aor ;
              if (participants[sender].name) opts.caller.name = participants[sender].name;
            }
            else {
              opts.callee.aor = participants[sender].aor ;
              if (participants[sender].name) opts.callee.name = participants[sender].name;
            }
          });
