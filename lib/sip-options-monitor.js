...
class SipOptionsMonitor {
  ...
  constructor(srf, config, logger, peerManager, metrics) {
    ...
    this.peerManager = peerManager || new PeerManager('./peers/peers.json', logger);
  ...
  syncPeers(peers) {
    ...
    for (const [name, peer] of newMap) {
      ...
      // updating a peer
      if (JSON.stringify(existing.peer) !== JSON.stringify(peer)) {
  ...
  startPeer(peer) {
    ...
    if (this.metrics) {
      const labels = { name: peer.name, role: peer.role || 'unknown', transport: peer.transport };
      rec(() => this.metrics.peerUp.set(labels, 0));
    }

    // Sequential loop for sending the SIP OPTIONS
    const loop = async () => {
  ...
  stopPeer(name) {
    ...
    if (this.metrics) {
      const { peer } = entry;
      const labels = { name: peer.name, role: peer.role || 'unknown', transport: peer.transport };
      rec(() => this.metrics.peerUp.remove(labels));
      rec(() => this.metrics.peerRttMs.remove(labels));
    }
  ...
  sendOptions(peer) {
    return new Promise((resolve) => {
      ...
      const mLabels = { name: peer.name, role: peer.role || 'unknown', transport: peer.transport };
      ...
      if (this.metrics) rec(() => this.metrics.peerOptionsSentTotal.inc({ name: peer.name, role: peer.role || 'unknown' }));
      ...
      const timer = setTimeout(() => {
        ...
        if (this.metrics) {
          rec(() => this.metrics.peerUp.set(mLabels, 0));
          rec(() => this.metrics.peerRttMs.set(mLabels, 0));
          rec(() => this.metrics.peerOptionsResultTotal.inc({ ...mLabels, result: 'timeout' }));
        }
        done();
      }, timeout);
        ...
        this.srf.request(
          ...
          (err, req) => {
            ...
            if (err) {
              ...
              if (this.metrics) {
                rec(() => this.metrics.peerUp.set(mLabels, 0));
                rec(() => this.metrics.peerOptionsResultTotal.inc({ ...mLabels, result: 'error' }));
              }
              return done();
            }
            ...
            req.on('response', (res) => {
              ...
              if (res.status !== 200) {
                ...
                if (this.metrics) {
                  rec(() => this.metrics.peerUp.set(mLabels, 0));
                  rec(() => this.metrics.peerOptionsResultTotal.inc({ ...mLabels, result: 'bad_status' }));
                }
                return done();
              }
              ...
              if (this.metrics) {
                rec(() => this.metrics.peerUp.set(mLabels, 1));
                rec(() => this.metrics.peerRttMs.set(mLabels, Math.round(rttMs)));
                rec(() => this.metrics.peerOptionsResultTotal.inc({ ...mLabels, result: 'ok' }));
              }
              done();
            });
          }
